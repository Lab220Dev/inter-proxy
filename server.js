...
async function proxy(res, fn) {
  try { const r = await fn(); res.status(r.status).send(r.body); }
  catch (err) { console.error('Erro:', err.message); res.status(500).json({ error: err.message }); }
}
function cobHeaders(token, extra = {}) { return { 'Authorization': `Bearer ${token}`, 'x-conta-corrente': CONTA, ...extra }; }
function bankHeaders(token) { return { 'Authorization': `Bearer ${token}`, 'x-conta-corrente': CONTA }; }
...
app.post('/cobranca/listar', async (req, res) => {
  ...
  await proxy(res, () => interRequest(`/cobranca/v3/cobrancas${qs({ dataInicial, dataFinal, filtrarDataPor, situacao, nome, cpfCnpjDevedor, itensPorPagina, paginaAtual })}`, 'GET', cobHeaders(token), null));
});
